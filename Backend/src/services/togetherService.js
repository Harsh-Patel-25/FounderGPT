...
function getClient() {
  if (!client) {
    if (!process.env.TOGETHER_API_KEY) {
      throw new Error("TOGETHER_API_KEY is not set. Add it to Backend/.env");
    }
    client = new OpenAI({
...
/**
 * Generic helper: sends a prompt to Together AI and returns parsed JSON.
 */
export async function askTogether(systemPrompt, userPrompt) {
  const openai = getClient();
  const model = process.env.TOGETHER_MODEL || "meta-llama/Llama-3-70b-chat-hf";
  ...
  try {
    ...
    logger.error("Together AI Error: " + err.message);
    throw err;
  }
}
...
export async function analyzeIdea(idea) {
  logger.info("Together AI → analyzeIdea");
  return askTogether(
...
export async function getCompetitors(idea) {
  logger.info("Together AI → getCompetitors");
  return askTogether(
...
export async function getBusinessModel(idea) {
  logger.info("Together AI → getBusinessModel");
  return askTogether(
...
export async function getRoadmap(idea) {
  logger.info("Together AI → getRoadmap");
  return askTogether(
...
export async function getPitch(idea) {
  logger.info("Together AI → getPitch");
  return askTogether(
...
export async function getStartupScore(idea) {
  logger.info("Together AI → getStartupScore");
  return askTogether(
...
export async function getCompleteDashboardData(idea) {
  logger.info("Together AI → getCompleteDashboardData (Unified)");
  return askTogether(
